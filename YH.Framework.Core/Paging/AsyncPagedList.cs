...
namespace YH.Framework.Core.Paging
{
    public class AsyncPagedList<T> : BasePagedList<T>
    {
        ...
        private async Task InitAsync(IQueryable<T> superset, int pageNumber, int pageSize)
        {
            ...
            {
                Subset.AddRange((pageNumber == 1)? await superset.Skip<T>(0).Take<T>(pageSize).ToListAsync<T>().ConfigureAwait(false): await superset.Skip<T>(((pageNumber - 1) * pageSize)).Take<T>(pageSize).ToListAsync<T>().ConfigureAwait(false));
            }
        }
    }
}
